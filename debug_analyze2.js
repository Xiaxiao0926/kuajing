...
function isSameSpec(supplierProduct, marketItem) {
    ...
    const marketBrand = marketItem.brand || extractBrand(marketItem.title);
    if (supplierBrand && marketBrand && supplierBrand.toLowerCase() !== marketBrand.toLowerCase()) return false;
    ...
    if (!supplierCapacity || !marketCapacity) return false;

    const supplierNum = getCapacityNumber(supplierCapacity);
    const marketNum = getCapacityNumber(marketCapacity);

    if (!supplierNum || !marketNum) return false;
...
function analyzePrices(supplierProducts, marketProducts) {
    ...
    for (const supplierProduct of supplierProducts) {
        ...
        for (const marketItem of marketProducts) {
            ...
            if (supplierProduct.name.includes('腾塔堡巴罗萨赤霞珠') && marketItem.price === 91 && marketItem.brand === '腾塔堡') {
        console.log('DBG: score=' + similarity.score + ' threshold=' + threshold + ' title=' + marketItem.title.substring(0,30) + ' passed=' + (similarity.score >= threshold));
    }
