...
const reports = generated.map((item) => item.report)
const hingeUrls = generated[0].urls
const overviewUrls = generated[2].urls
const overlapCount = [...hingeUrls].filter((url) => overviewUrls.has(url)).length
reports[0].insights.push(`与 9 月 7 日家具五金综合样本共有 ${overlapCount} 个商品链接；两份数据覆盖范围不同，应分别看作专项样本与综合样本，而不是重复报告。`)
reports[2].insights.push(`与 9 月 8 日合页专项样本共有 ${overlapCount} 个商品链接；综合样本适合找方向，专项样本适合继续拆规格与竞品。`)
...
console.log(`Generated ${reports.length} recent Ozon reports in ${OUTPUT_PATH}`)
