...
## 2026-09-10（市场报告俄罗斯维度深化）

### feat — 24 份市场报告统一升级
- 报告首屏新增 `Executive Summary / 执行摘要`，先呈现需求集中点、新品与成熟品差异及俄罗斯市场进入判断，再展开样本指标。
- 新增俄罗斯市场进入模块：2026 年上半年线上零售背景、近似大类口径、品牌/卖家 HHI、0–180 天新品与成熟品队列、体积与价值密度、品类探索优先分、法规预筛和证据缺口。
- 品类探索分固定为样本内排序：需求 40% / 新品 20% / 供给缺口 15% / 未知品牌进入线索 15% / 物流测试性 10%；不可跨报告比较，不解释为销量、利润或成功概率。
- 俄罗斯宏观数据与 Ozon BSR 样本保持独立，明确禁止把全网大类份额直接套算为平台子类目规模；法规结果仅作打样前预筛。

### fix — 报告手机端横向溢出
- 静态报告图表卡和俄罗斯双栏面板增加最小宽度约束；动态报告中的宽表只在自身容器内横向滚动，不再撑宽整页。
- 修正车灯照明长报告将 `33.1%` 误写为 `330%` 的文案，并统一取整品牌溢价图表的百分比刻度、提示和标签，避免浮点尾数进入公开页面。

### test — 报告质量门
- 新增 24 份报告统一检查：执行摘要、俄罗斯来源、已知数据缺口、建议证据、180 天新品窗口、安全链接、探索分范围与决策枚举。
- `npm test`、`npm run test:golden`、`npm run test:sync`、`npm run test:web-persistence` 与 Vite 生产构建全绿；桌面和 390px 手机视口人工验收通过。
