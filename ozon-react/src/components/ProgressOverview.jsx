...
const COLOR_MAP = {
  rose: { bg: 'bg-workspace-surface-subtle', border: 'border-rose-200', text: 'text-workspace-text', accent: 'bg-workspace-surface-subtle0', light: 'bg-workspace-surface-subtle', bar: 'bg-rose-400' },
  ...
  purple: { bg: 'bg-workspace-surface-subtle', border: 'border-workspace-border', text: 'text-workspace-text', accent: 'bg-workspace-surface-subtle0', light: 'bg-workspace-surface-subtle', bar: 'bg-purple-400' },
  green: { bg: 'bg-workspace-success-soft', border: 'border-green-200', text: 'text-workspace-success', accent: 'bg-workspace-success-soft0', light: 'bg-workspace-success-soft', bar: 'bg-green-400' },
}

const STATUS_DOT = {
  done: 'bg-workspace-success-soft0',
  progress: 'bg-workspace-warning-soft0',
  pending: 'bg-gray-300',
}
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-morandi-text-light mt-0.5">按产品品类跟踪各阶段进度</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-gradient-to-br from-blue-50 to-blue-100/50 rounded-xl p-4 border border-blue-100">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <span className="text-xs font-medium text-workspace-text-secondary">产品线</span>
          </div>
          <div className="text-2xl font-bold text-workspace-primary">{PRODUCTS.length}</div>
          <div className="text-xs text-blue-400 mt-0.5">个品类并行</div>
        </div>
        <div className="bg-gradient-to-br from-green-50 to-green-100/50 rounded-xl p-4 border border-green-100">
          <div className="flex items-center gap-1.5 mb-2">
            <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />
            <span className="text-xs font-medium text-workspace-success">总完成率</span>
          </div>
          <div className="text-2xl font-bold text-workspace-success">{totalAll > 0 ? Math.round(doneAll / totalAll * 100) : 0}%</div>
          <div className="text-xs text-green-400 mt-0.5">{doneAll}/{totalAll} 节点</div>
        </div>
        <div className="bg-gradient-to-br from-amber-50 to-amber-100/50 rounded-xl p-4 border border-workspace-border">
          <div className="flex items-center gap-1.5 mb-2">
            <TrendingUp className="w-3.5 h-3.5 text-amber-500" />
            <span className="text-xs font-medium text-amber-600">推进中</span>
          </div>
          <div className="text-2xl font-bold text-workspace-warning">{allStats.reduce((s, p) => s + p.stats.progress, 0)}</div>
          <div className="text-xs text-amber-400 mt-0.5">个节点执行中</div>
        </div>
        <div className="bg-gradient-to-br from-red-50 to-red-100/50 rounded-xl p-4 border border-red-100">
          <div className="flex items-center gap-1.5 mb-2">
            <Target className="w-3.5 h-3.5 text-red-500" />
            <span className="text-xs font-medium text-red-600">瓶颈</span>
          </div>
          <div className="text-sm font-bold text-red-700 truncate">{bottleneck ? `${bottleneck.product}` : '无'}</div>
          <div className="text-xs text-red-400 mt-0.5 truncate">{bottleneck ? `${bottleneck.phase} ${bottleneck.pct}%` : '进度正常'}</div>
        </div>
      </div>
          ...
          {allStats.map(p => {
            ...
            return (
              <div key={p.id} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-3 bg-workspace-surface-subtle rounded-full overflow-hidden">
                ...
                <span className="text-xs text-morandi-text-light w-16 text-right">{p.stats.done}/{p.stats.total}</span>
              </div>
            )
          })}
        </div>
      ...
      <div className="space-y-3">
        {PRODUCTS.map(product => {
          ...
          return (
            <div key={product.id} className={`bg-white rounded-xl shadow-sm border overflow-hidden transition-all ${isExpanded ? `${colors.border}` : 'border-workspace-border'}`}>
              <button
                onClick={() => setExpandedProduct(isExpanded ? null : product.id)}
                className="w-full px-5 py-4 flex items-center justify-between hover:bg-workspace-surface-subtle/50 transition-colors"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-morandi-text-light">{stats.done}/{stats.total} 节点已完成 · {stats.progress} 进行中</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-32 h-2 bg-workspace-surface-subtle rounded-full overflow-hidden">
              ...
              {isExpanded && (
                <div className="px-5 pb-4 space-y-3">
                  {product.phases.map((phase, pi) => {
                    ...
                    return (
                      <div key={pi} className="border border-workspace-border rounded-lg overflow-hidden">
                        <div className={`px-4 py-2.5 flex items-center justify-between ${phaseStats.pct === 100 ? 'bg-workspace-success-soft/50' : 'bg-workspace-surface-subtle/50'}`}>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`text-xs px-1.5 py-0.5 rounded-full ${phaseStats.pct === 100 ? 'bg-workspace-success-soft text-workspace-success' : phaseStats.pct > 0 ? 'bg-workspace-warning-soft text-workspace-warning' : 'bg-workspace-surface-subtle text-workspace-text-tertiary'}`}>
                          ...
                          {phase.nodes.map(nodeId => {
                            ...
                            return (
                              <button
                                ...
                                className="flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-workspace-surface-subtle transition-colors text-left group"
                              >
                                <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${STATUS_DOT[status]}`} />
                                <span className="text-xs text-morandi-text group-hover:text-morandi-primary truncate">{title}</span>
