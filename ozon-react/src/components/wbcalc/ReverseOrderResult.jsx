    ...
    <div className="bg-orange-50/50 border border-orange-200 rounded-lg p-3 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1">
          {labels.map((lbl, i) => (
            <span key={i} className={`text-xs px-2 py-0.5 rounded-full border ${labelColorMap[lbl.color] || labelColorMap.gray}`}>
              {lbl.text}
            </span>
          ))}
        </div>
      ...
      {result.needsBillConfirmation && (
        <div className="rounded bg-yellow-50 border border-yellow-300 p-2 flex items-center gap-2">
          ...
          <span className="text-xs text-yellow-700">⚠️ 此场景暂按 {result.multiplier}×CSG 测算，需以WB实际账单确认为准</span>
        </div>
      )}
      ...
      {showSteps && (
        <div className="bg-gray-50 rounded p-2 max-h-48 overflow-y-auto">
          {result.steps.map((s, i) => (
            <p key={i} className={`text-xs text-gray-600 ${s.startsWith('=====') ? 'font-semibold mt-2' : ''}`}>{s}</p>
          ))}
        </div>
      )}
    </div>
