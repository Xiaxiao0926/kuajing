...
function analyzePrices(supplierProducts, marketProducts) {
    ...
    for (const supplierProduct of supplierProducts) {
        ...
        if (supplierProduct.name.includes('腾塔堡巴罗萨赤霞珠')) {
            console.log('AFTER SORT top5:');
            matches.slice(0, 5).forEach((m, i) => {
                console.log('  ' + i + ': sim=' + m.similarity + ' price=' + m.price + ' sameSpec=' + isSameSpec(supplierProduct, m) + ' review=' + (m.needsReview||false) + ' ' + m.title.substring(0, 30));
            });
        }
